...
def get_equations():
    ...
    for target, replacement in accumulated_subs:
        eq_Sigma_dot = eq_Sigma_dot.subs(target, replacement)
    ...
    for target, replacement in accumulated_subs:
        eq_B_dot = eq_B_dot.subs(target, replacement)
    ...
    for target, replacement in accumulated_subs:
        eq_A = eq_A.subs(target, replacement)

    return {
        "G_uv_list": G_uv_list,
        "eq_Sigma": clean_equation(eq_Sigma),
        "eq_F": clean_equation(eq_F),
        "eq_Sigma_dot": clean_equation(eq_Sigma_dot),
        "eq_B_dot": clean_equation(eq_B_dot),
        "eq_A": clean_equation(eq_A),
        "substitutions": list(accumulated_subs),
    }
...
def run_main():
    ...
    # (2.8a) G_22 -> Sigma
    print("\n(2.8a) Extracting Sigma Equation from G_22 (G_rr)")
    eq_Sigma = clean_equation(apply_dot_operator(G_uv_list[2][2]))
    ...
    print("\n(2.8c) Extracting Sigma_dot Equation")
    eq_Sigma_dot = clean_equation(apply_dot_operator(G_uv_list[0][2])) / 2
